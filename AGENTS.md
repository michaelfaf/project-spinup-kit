...
2. **`INSTALL-STATUS.md` is your memory.** Check off each task as it completes and record every decision the user makes in the Decisions table. If this session dies, the next session (you or another agent) resumes from `INSTALL-STATUS.md` — so keep it current as you go, not at the end.
